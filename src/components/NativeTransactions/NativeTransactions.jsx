...
function NativeTransactions() {
  ...
  const columns = [
    ...
    },
    {
      render: (_, {tags}) => {
        return (
          tags.some(tag => tag === walletAddress) ? 
          <span className={styles.btnDelete} onClick={() => handleDelistClick(_)}>
            <DeleteOutlined />
          </span>
          : null
        )
  ...
  return (
    <div style={{ display: 'flex', position: 'relative' }}>
      ...
      <Modal
        ...
        footer={[
          <Button key="1" loading={loading} className={styles.btnCancel} onClick={() => setVisibility(false)}>Cancel</Button>,
          // <Button key="2" type="primary" onClick={() => approve()}>Approve</Button>,
          <Button key="3" loading={loading} className={styles.btnDelist} type="primary" onClick={() => approveAll()}>Delist</Button>
        ]}
      >
